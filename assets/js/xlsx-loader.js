...
function buildCardIconHtml(id, prefeb) {
  ...
  // 2026-09-13修正(Wei回報戰鬥加速時card_prefeb特效播放速度也跟著變快)：戰鬥中renderRow()每次更新都會
  // 整個重建卡片DOM(含這個icon)，導致CSS animation每次都從頭重播——戰鬥加速時render頻率變高，animation
  // 被重置的頻率也跟著變高，畫面上看起來就像特效被加速播放，但實際上animation本身的秒數從頭到尾都沒有變。
  // 改用「距離戰鬥開始的實際經過時間(真實時間，完全不受戰鬥速度倍率影響)」算出一個負的animation-delay，
...
function buildStoryLines(wb) {
  ...
  // 2026-09-12新增：教官職業介紹(classIntro)、艾琳召喚武器前置對話(gachaIntro，從「玩家取名字後」到
  // 『勇者再次握住武器，而沉睡的身體本能，似乎認出了它。』這句為止)這兩段的background欄位，比照上面
  // mapid=100那三段的做法一併讀出，讓這兩段教學對話的背景圖也能統一由text分頁的background欄位控制。
  if (guideDialogue[0] && guideDialogue[0].background) out.classIntroBg = String(guideDialogue[0].background).trim();
  if (guideDialogue[1] && guideDialogue[1].background) out.gachaIntroBg = String(guideDialogue[1].background).trim();
  return out;
}
